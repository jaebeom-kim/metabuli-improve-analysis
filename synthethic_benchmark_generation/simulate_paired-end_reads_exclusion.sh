...
if [ -z "$FASTA_PATHS" ] || [ -z "$QUERY_ACC" ] || [ -z "$OUTDIR" ] || [ -z "$COVERAGE" ]; then
    echo "Usage: $0 <fasta_file_list> <exclusion_file> <output_directory> <coverage>"
    exit 1
fi
...
awk -F '\t' '{print $NF}' $QUERY_ACC | grep "^GC" > query_accession_list.txt
...
while read -r FASTA; do
    ...
    # CALCULATE READ_NUM USING AWK (Handles decimals and rounds to nearest integer)
    READ_NUM=$(awk -v size="$GENOME_SIZE" -v cov="$COVERAGE" 'BEGIN { printf "%.0f", (size * cov) / 300 }')
