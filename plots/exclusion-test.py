...
def prokaryote_all():
    ...
    fig, axs = plt.subplots(1, 4, sharex='all', sharey='all', figsize=(12, 10), gridspec_kw={'hspace': 0.5, 'wspace': 0.1})
    # Read data
    # subspecies exclusion
    subsp_ex_short_data = pd.read_csv('subspecies-ex-score.tsv', sep='\t')
   
    # species exclusion
    sp_ex_short_data = pd.read_csv('species-ex-score.tsv', sep='\t')

    # genus exclusion
    genus_ex_short_data = pd.read_csv('genus-ex-score.tsv', sep='\t')

    # family exclusion
    family_ex_short_data = pd.read_csv('family-ex-score.tsv', sep='\t')

    # Scatter plot parameters
    order = ["Metabuli*", "Metabuli",
             'Kraken2', 'Centrifuger',
             'Kraken2X', 'Kaiju']
    markers = ['o', 's',
               'D', 'v',
    ...
    titles = [['Illumina', 'PacBio HiFi', 'ONT', 'PacBio Sequel II'],
              ...
              ['Illumina', 'PacBio HiFi', 'ONT', 'PacBio Sequel II']]
    data = [[subsp_ex_short_data, sp_ex_short_data, genus_ex_short_data, family_ex_short_data]]
    panels = [['a', 'b', 'c', 'd']]
    x_pos = 0
    y_pos = 1.1
